...
export default function Task({ item, allComments }: TaskProps) {
  ...
  async function deleteButtonTrash(id: string) {
    try{
        ...
        {comments.map((item) => (
          <article key={item.id} className={styles.comment}>
            <div className={styles.headComments}>
               <label className={styles.CommentsName}>{item.name}</label>
               {item.user === session?.user?.email && (
                 <button className={styles.commentsButtonTrash}onClick={()=>deleteButtonTrash(item.id)}>
                 <FaTrash
                 size={18}
                 color="#ea3140"/>
                </button>
